...
// some minor modifications and casts to floats Pete Cooke Sept 2016
...
float length(const vec3& v) {
	return static_cast<float>(sqrt(v.v[0] * v.v[0] + v.v[1] * v.v[1] + v.v[2] * v.v[2]));
}
...
float direction_to_heading(vec3 d) {
	return static_cast<float>(atan2(-d.v[0], -d.v[2]) * ONE_RAD_IN_DEG);
}

vec3 heading_to_direction(float degrees) {
	float rad = static_cast<float>(degrees * ONE_DEG_IN_RAD);
...
mat4 rotate_x_deg(const mat4& m, float deg) {
	// convert to radians
	float rad = static_cast<float>(deg * ONE_DEG_IN_RAD);
	mat4 m_r = identity_mat4();
	m_r.m[5] = static_cast<float>(cos(rad));
	m_r.m[9] = static_cast<float>(-sin(rad));
	m_r.m[6] = static_cast<float>(sin(rad));
	m_r.m[10] = static_cast<float>(cos(rad));
	return m_r * m;
}
...
mat4 rotate_y_deg(const mat4& m, float deg) {
	...
	float rad = static_cast<float>(deg * ONE_DEG_IN_RAD);
	mat4 m_r = identity_mat4();
	m_r.m[0] = static_cast<float>(cos(rad));
	m_r.m[8] = static_cast<float>(sin(rad));
	m_r.m[2] = static_cast<float>(-sin(rad));
	m_r.m[10] = static_cast<float>(cos(rad));
	return m_r * m;
}
...
mat4 rotate_z_deg(const mat4& m, float deg) {
	...
	mat4 m_r = identity_mat4();
	m_r.m[0] = static_cast<float>(cos(rad));
	m_r.m[4] = static_cast<float>(-sin(rad));
	m_r.m[1] = static_cast<float>(sin(rad));
	m_r.m[5] = static_cast<float>(cos(rad));
	return m_r * m;
}
...
mat4 perspective(float fovy, float aspect, float zNear, float zFar) {
	float fov_rad = static_cast<float>(fovy * ONE_DEG_IN_RAD);
	float range = static_cast<float>(tan(fov_rad / 2.0f) * zNear);
...
versor quat_from_axis_rad(float radians, float x, float y, float z) {
	versor result;
	result.q[0] = static_cast<float>(cos(radians / 2.0));
	result.q[1] = static_cast<float>(sin(radians / 2.0) * x);
	result.q[2] = static_cast<float>(sin(radians / 2.0) * y);
	result.q[3] = static_cast<float>(sin(radians / 2.0) * z);
	return result;
}

versor quat_from_axis_deg(float degrees, float x, float y, float z) {
	return quat_from_axis_rad(static_cast<float>(ONE_DEG_IN_RAD * degrees), x, y, z);
}
...
versor normalise(versor& q) {
	...
	float mag = static_cast<float>(sqrt(sum));
	return q / mag;
}
...
versor slerp(versor& q, versor& r, float t) {
	...
	float sin_half_theta = static_cast<float>(sqrt(1.0f - cos_half_theta * cos_half_theta));
	...
	float half_theta = static_cast<float>(acos(cos_half_theta));
	float a = static_cast<float>(sin((1.0f - t) * half_theta) / sin_half_theta);
	float b = static_cast<float>(sin(t * half_theta) / sin_half_theta);
	for (int i = 0; i < 4; i++) {
